...
def generate_url(categories):
    for category in categories['신발']:
        for color in shoes_color_dict.keys():
            url = f'https://api.musinsa.com/api2/dp/v1/plp/goods?gf=M&color={shoes_color_dict[color]}&sortCode=REVIEW&category={category[1]}&size=60&caller=CATEGORY&page=1'
            # print(url)
        ...
        for style in total_style_dict.keys():
            for color in total_color_dict.keys():
                for fit in bottom_fit_dict.keys():
                    url = f'https://api.musinsa.com/api2/dp/v1/plp/goods?gf=M&color={total_color_dict[color]}&attribute={bottom_fit_dict[fit]}&style={total_style_dict[style]}&sortCode=REVIEW&category={category[1]}&size=60&caller=CATEGORY&page=1'
                    # print(url)
        ...
        for style in total_style_dict.keys():
            for color in total_color_dict.keys():
                for fit in top_fit_dict.keys():
                    url = f'https://api.musinsa.com/api2/dp/v1/plp/goods?gf=M&color={total_color_dict[color]}&attribute={top_fit_dict[fit]}&style={total_style_dict[style]}&sortCode=REVIEW&category={category[1]}&size=60&caller=CATEGORY&page=1'
                    # print(url)
                ...
                for color in total_color_dict.keys():
                    for fit in top_fit_dict.keys(): 
                        for sleeve in sleeve_dict.keys():
                            url = f'https://api.musinsa.com/api2/dp/v1/plp/goods?gf=M&color={total_color_dict[color]}&attribute={top_fit_dict[fit]},{sleeve_dict[sleeve]}&style={total_style_dict[style]}&sortCode=REVIEW&category={category[1]}&size=60&caller=CATEGORY&page=1'
                            # print(url)
